(function() {
	...
	window.onload = function() {
		...
		/**
		 * Draws the listing to the DOM.
		 *
		 * @param {Listing} listing The listing to draw.
		 */
		function drawListing(listing) {
			// create a new DOM element
			let element = document.createElement('div');
			element.className = 'listing-info';
			element.id = 'listing-' + listing._id;
			element.innerHTML=`
				<div class="well listing-well">
					<a href="/listings/${listing._id}/details" class="listing-tile">
						<img class="listing-image" 
							src="/api/listings/${listing._id}/picture/" 
							alt="${listing.title}" />
						<div class="listing-title two-line-ellipsis">${listing.title}</div>
						<div class="listing-price">
							C $${(listing.price / 100).toFixed(2)}
						</div>
					</a>
				</div>`;

			document.querySelector('#listing-grid').append(element);
		}

		api.getListings(0, function(err, listings) {
